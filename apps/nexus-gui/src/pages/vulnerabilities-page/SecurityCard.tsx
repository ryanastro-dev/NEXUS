...
import { classifyDeviceRisk } from './helpers';
...
export function SecurityCard({ device }: SecurityCardProps) {
  ...
  const isSecure = classifyDeviceRisk(device) === 'secure';
  ...
  const hostForDrillDown: HostInfo = {
    ip: device.last_ip,
    mac: device.mac,
    ...
    risk_score: Number.isFinite(device.risk_score) ? (device.risk_score as number) : 0,
    open_ports: [],
    vulnerabilities,
    port_warnings: portWarnings,
    security_grade: device.security_grade ?? 'N/A',
  };
  ...
  return (
    <div className="flex h-full min-h-[236px] flex-col rounded-2xl border border-slate-200/70 bg-white/85 p-3.5 shadow-sm backdrop-blur-sm transition-all hover:border-accent-blue/30 dark:border-slate-800 dark:bg-slate-950/65">
      <div className="mb-3 flex items-start justify-between">
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-sm text-text-muted">
            {device.vendor || vulnerabilitiesCopy.card.unknownVendor}
          </p>
          <div className="mt-1.5 flex items-center gap-2 text-xs text-text-muted">
            <span>{vulnerabilitiesCopy.card.ip} {device.last_ip}</span>
            <span>•</span>
            <span>{vulnerabilitiesCopy.card.mac} {device.mac}</span>
          </div>
        </div>
        ...
        <div className="mb-2.5">
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <h4 className="text-xs font-bold uppercase tracking-wide text-accent-red">
              {vulnerabilitiesCopy.card.knownVulnerabilities.replace(
                '{count}',
                String(vulnerabilities.length),
              )}
            </h4>
          </div>
                ...
                <div className="mb-1 flex items-start justify-between gap-2">
                  ...
                  {vulnerability.cvss_score && (
                    <span className="rounded bg-accent-red/10 px-1.5 py-0.5 text-[10px] font-bold text-accent-red">
                      {vulnerabilitiesCopy.card.cvss} {vulnerability.cvss_score.toFixed(1)}
                    </span>
                  )}
                </div>
        ...
        <div className="mb-2.5">
          <div className="mb-2 flex items-center gap-1.5">
            ...
            <h4 className="text-xs font-bold uppercase tracking-wide text-accent-blue">
              {vulnerabilitiesCopy.card.portSecurityWarnings.replace(
                '{count}',
                String(portWarnings.length),
              )}
            </h4>
          </div>
      ...
      {isSecure && (
        <div className="mt-auto py-6 text-center">
          ...
          <h4 className="mb-1 text-lg font-bold text-text-primary">
            {vulnerabilitiesCopy.card.allClearTitle}
          </h4>
          <p className="text-sm text-text-muted">
            {vulnerabilitiesCopy.card.allClearDescription}
          </p>
        </div>
      )}
      ...
      <div className="mt-auto pt-2">
        <button
          onClick={() => openDeviceDetails(hostForDrillDown)}
          className="inline-flex items-center rounded-lg border border-cyan-300 bg-cyan-100 px-3 py-1.5 text-xs font-semibold text-cyan-800 transition-colors hover:bg-cyan-200 dark:border-cyan-700 dark:bg-cyan-900/40 dark:text-cyan-200 dark:hover:bg-cyan-900/60"
        >
          {vulnerabilitiesCopy.card.openDrillDown}
          {hiddenFindingCount > 0 ? (
            <span className="ml-1.5 rounded-md bg-white/75 px-1.5 py-0.5 text-[10px] text-cyan-700 dark:bg-slate-900 dark:text-cyan-200">
              {vulnerabilitiesCopy.card.moreFindings.replace('{count}', String(hiddenFindingCount))}
            </span>
          ) : null}
        </button>
      </div>
