...
#[tauri::command]
pub async fn ai_analyze_device_security(device: HostInfo) -> CommandResult<DeviceSecurityAnalysis> {
    commands::ai_analyze_device_security_impl(device).await
}
...
pub async fn ai_generate_network_report(
    ...
    commands::ai_generate_network_report_impl(state, hosts, subnet).await
}
...
pub async fn ai_troubleshoot_device(
    ...
    commands::ai_troubleshoot_device_impl(device, symptoms).await
}
